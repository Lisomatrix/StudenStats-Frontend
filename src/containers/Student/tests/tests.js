...
const NestedTable = props => {
  const expandedRowRender = gradeRows => {
    ...
    return (
      <Table columns={columns} dataSource={gradeRows} pagination={false} />
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "media",
      render: (text, row, index) => {
        return <span>{text.media !== 0.0 ? "0" : text.media}</span>;
      }
    },
    {
...
class StudentTests extends React.Component {
  ...
  componentWillMount() {
    if (!dataFetched) {
      ...
      this.props.requestStudentTestGrades();
    }
  }
  ...
  _requestNeededData = (nextProps, nextState) => {
    if (
      ...
      !nextState.requestedClassDisciplines
    ) {
    ...
    if (
      !nextProps.modules &&
      nextProps.studentClass &&
      !nextState.requestedClassModules
    ) {
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    if (
      ...
      for (var i = 0; i < nextProps.moduleGrades.length; i++) {
        const foundModule = this._getModule(
          nextProps.moduleGrades[i],
          nextProps.modules
        );

        if (
          foundModule.moduleId !== nextState.selectedModule &&
          nextState.selectedModule !== -1
        ) {
          continue;
        }

        const discipline = this._getSelectedDiscipline(
          foundModule.disciplineId,
          nextProps.disciplines
        );

        if (
          discipline.disciplineId !== nextState.selectedDiscipline &&
          nextState.selectedDiscipline !== -1
        ) {
          continue;
        }
        ...
        const moduleGrade = this._getModuleGrade(
          ...
          <div className="students-tests-options">
            ...
            <div className="select select-margin">
              ...
              <Select
                ...
                onChange={value => this.setState({ selectedModule: value })}
                placeholder="Modules..."
              >
                {avaibleModules}
                <Option value={-1}>Todos</Option>
